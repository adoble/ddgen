...
// Ignoring this test as the code for embassy-rp  seems to  only supports SpiBus for embedded-hal v1.0.0
// and not SpiDevice.
#[test]
#[ignore]
fn spi_device_test() {
    ...
    // Writing
    SpiDevice::write(&mut spi, &vec![1, 2]).unwrap();

    // Transferring
    let mut buf = vec![3, 4];
    //spi.transfer_in_place(&mut buf).unwrap();
    SpiDevice::transfer_in_place(&mut spi, &mut buf).unwrap();
    assert_eq!(buf, vec![5, 6]);
